...
NetworkTrainer<NetworkType, ProgressEvaluator, CostFunction>::NetworkTrainer(int num_epochs_new, int samples_per_epoch_new, double learning_rate_new, double test_data_frac_new, std::shared_ptr<NetworkType> network_new)
	: num_epochs(num_epochs_new), samples_per_epoch(samples_per_epoch_new), learning_rate(learning_rate_new), test_data_frac(test_data_frac_new), network(network_new) {

}

template<class NetworkType, class ProgressEvaluator, class CostFunction>
void NetworkTrainer<NetworkType, ProgressEvaluator, CostFunction>::train(const SentenceList& sl) {
	// Figure out how many test/training examples we will have
	int num_test_exs = sl.size() * test_data_frac;
	int num_training_exs = sl.size() - num_test_exs;
	...
	std::vector<int> randomized_examples(sl.size());
	...
	for(int random_ex_num = 0; random_ex_num < sl.size(); random_ex_num++) {
	...
	for(int epoch = 0; epoch < num_epochs; epoch++) {
		...
		for(int batch = 0; batch < num_batches_per_epoch; batch++) {
			...
			for(int ex = 0; ex < num_examples_in_batch; ex++) {
				...
				network->feedForward(sl[exNum], saved_states, outputs);
				addGradients(sl[exNum], *saved_states, *outputs, dCdW, dCdU, dCdV);
			}
